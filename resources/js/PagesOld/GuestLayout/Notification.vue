    ...
    >
        ...
        <div v-for="(notification, index) in notifications" :key="notification.id">
          <div v-if="index < 6">
            ...
            <p class="mb-2">
              <i
                :class="
                  'icofont-ui-press food-item ' +
                  (notification.read_at ? 'text-success' : 'text-danger')
                "
              ></i
              >Order {{ "#" + notification.data.info.order.id }} is
              {{ getOrderStatus(notification.data.info.status) }}
      ...
      <div class="dropdown-cart-top-footer border-top p-2">
        <a class="btn btn-success btn-block btn-lg" href="#"
          >{{ $t("listing.view_all") }} {{ notifications.length }}
          {{ $t("listing.notifications") }}</a
        >
      </div>
    </div>
  </div>
...
export default {
  mounted() {
    Echo.private("App.Models.User." + this.$page.props.auth.id).notification(
      (notification) => {
