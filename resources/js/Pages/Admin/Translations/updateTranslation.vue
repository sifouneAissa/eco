<template>
    ...
    <div :key="keyV"  class="mt-2 ml-2">
        <div class="ml-auto">
            <a class="text-warning" data-toggle="collapse" :href="'#collapseExample'+keyV" role="button" aria-expanded="false" :aria-controls="'collapseExample'+keyV"><i class="fa fa-plus-circle"></i> Edit {{attr}} translations</a>
        </div>
...
<script >
    export default {
            ...
            writing (){
                ...
                let data = this.$page.props.locales.filter((item) => item!=='en').map((lang) => {
                    return {
                        ...
                        id : this.model[this.keyV+'_'+lang]?.id
                    }
                })
        ...
        mounted() {
            if(this.model)
                this.$page.props.locales.map((lang) => {
                    this.data[lang] = this.model[this.keyV+'_'+lang]?.value;
                });
        },
        data () {
            return {
