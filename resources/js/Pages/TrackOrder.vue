            ...
            <div class="row d-flex align-items-center">
                ...
                <div class="col-md-6">
                    <div class="bg-white p-4 shadow-lg mb-2">
                        <div class="mb-2"><small>Order {{order.order_number}}<a class="float-right font-weight-bold" href="#"><i class="icofont-headphone-alt"></i> HELP</a></small></div>
                        <h6 class="mb-1 mt-1"><a href="detail.html" class="text-black">{{order.names}}
                        </a>
                        </h6>
                        <p class="text-gray mb-0"><i class="icofont-clock-time"></i> 04:19 PM | {{order.products.length}} Items | {{$page.props.currency_code}} {{order.price}}</p>
                    </div>
                    <div class="bg-white p-4 shadow-lg">
                        <div class="osahan-track-order-detail po">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-7">
                                    <div class="mb-2"><small><i class="icofont-list"></i> {{order.products.length}} ITEMS</small></div>

                                    <div v-for="product in order.order_items" :key="product.id">
                                        <p class="mb-2"><i class="icofont-ui-press text-success food-item"></i> {{product.product.name}} X {{product.quantity}}   <span class="float-right text-secondary">{{$page.props.currency_code}} {{product.product.cprice}}</span></p>
                                    </div>
                                    <p class="mb-0 font-weight-bold text-black">TOTAL BILL  <span class="float-right text-secondary">{{$page.props.currency_code}}  {{order.price}}</span></p>
                    ...
                    <div class="bg-white p-4 shadow-lg mt-2">
                        <div class="row text-center">
                            <div class="col">
                                ...
                                <small class="text-info mb-0" v-if="!order.received">NOW</small>
                                <small class="text-success mb-0" v-else>Checked</small>
                            </div>
                            <div class="col">
                                <i class="icofont-check-circled icofont-3x text-success"></i>
                                <p class="mt-1 font-weight-bold text-dark mb-0">Order Confirmed</p>
                                <small class="text-info mb-0" v-if="!order.confirmed && order.received">NOW</small>
                                <small class="text-primary mb-0" v-else-if="!order.confirmed && !order.received">Later</small>
                                <small class="text-success mb-0" v-else>Checked</small>
                            </div>
                            <div class="col">
                                <i class="icofont-delivery-time icofont-3x text-primary"></i>
                                <p class="mt-1 font-weight-bold text-dark mb-0">Order Picked Up</p>
                                <small class="text-info mb-0" v-if="!order.picked_up && order.confirmed">NOW</small>
                                <small class="text-primary mb-0" v-else-if="!order.picked_up && !order.confirmed">Later</small>
                                <small class="text-success mb-0" v-else>Checked</small>
                            </div>
                        </div>
                    </div>
