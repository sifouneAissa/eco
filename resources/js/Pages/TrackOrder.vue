      ...
      <div class="row d-flex align-items-center">
        ...
        <div class="col-md-12">
          <div class="bg-white p-4 shadow-lg mb-2">
            ...
            <h6 class="mb-1 mt-1">
              <div class="text-black">{{ order.names }}</div>
            </h6>
            <p class="text-gray mb-0">
              <i class="icofont-clock-time"></i> {{ order.created_date }} |
              {{ order.products.length }} {{ $t("track_order.items") }} |
              {{ $page.props.currency_code }} {{ order.price }}
            </p>
          </div>
            ...
            <div class="osahan-track-order-detail po">
              ...
              <div class="row">
                ...
                <div class="col-md-12">
                  <div class="mb-2">
                    <small
                      ><i class="icofont-list"></i> {{ order.products.length }}
                      {{ $t("track_order.items") }}</small
                    >
                  </div>
                  ...
                  <div v-for="product in order.order_items" :key="product.id">
                    <p class="mb-2">
                      ...
                      <span class="float-right text-secondary"
                        ...
                        {{ product.product.cprice }}</span
                      >
                    </p>
                  </div>
                  <p class="mb-0 font-weight-bold text-black">
                    {{ $t("track_order.total_bill") }}
                    <span class="float-right text-secondary"
                      >{{ $page.props.currency_code }} {{ order.price }}</span
                    >
                  </p>
            ...
            <div class="row text-center">
              <div class="col">
                ...
                <p class="mt-1 font-weight-bold text-dark mb-0">
                  {{ $t("track_order.order_received") }}
                </p>
                <small class="text-info mb-0" v-if="!order.received">{{
                  $t("track_order.now")
                }}</small>
                <small class="text-success mb-0" v-else>{{
                  $t("track_order.checked")
                }}</small>
              </div>
              <div class="col">
                <i class="icofont-check-circled icofont-3x text-success"></i>
                <p class="mt-1 font-weight-bold text-dark mb-0">
                  {{ $t("track_order.order_confirmed") }}
                </p>
                <small class="text-info mb-0" v-if="!order.confirmed && order.received">{{
                  $t("track_order.now")
                }}</small>
                <small
                  class="text-primary mb-0"
                  v-else-if="!order.confirmed && !order.received"
                  >{{ $t("track_order.later") }}</small
                >
                <small class="text-success mb-0" v-else>{{
                  $t("track_order.checked")
                }}</small>
              ...
              <div class="col">
                ...
                <p class="mt-1 font-weight-bold text-dark mb-0">
                  {{ $t("track_order.order_picked_up") }}
                </p>
                <small
                  class="text-info mb-0"
                  v-if="!order.picked_up && order.confirmed"
                  >{{ $t("track_order.now") }}</small
                >
                <small
                  class="text-primary mb-0"
                  v-else-if="!order.picked_up && !order.confirmed"
                  >{{ $t("track_order.later") }}</small
                >
                <small class="text-success mb-0" v-else>{{
                  $t("track_order.checked")
